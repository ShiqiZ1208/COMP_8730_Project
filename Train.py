...
############################################ Training The Generator ####################################################
            ...
            if BART_only == True:
              ...
              loss3 = output_g.loss + output_fd.loss
            loss3.backward()
        ...
        epochs += 1
        if BART_only ==False:
          NetG.load_state_dict(torch.load(f"./BARTmodel/lora_bartGAN_{epoch}_{minibatch_sizes}.pth"))
        else:
          NetG.load_state_dict(torch.load(f"./BARTmodel/lora_bart_{epoch}_{minibatch_sizes}.pth"))
